...
export function AgentsPage() {
  ...
  const handleViewAgent = (agentId: string) => {
    router.push(`/agents/${agentId}?mode=edit&tab=configure`);
  };

  // 处理 AI 编辑智能体
  const handleEditAgentWithAI = useCallback(
    (agent: AgentDef) => {
      router.push(`/agents/${agent.id}?mode=edit&tab=ai-create`);
    },
    [router],
  );
          ...
          <div className="flex gap-1">
            ...
            {allExpertises.slice(0, 5).map((expertise) => (
              <Badge
                key={expertise}
                variant={
                  selectedExpertise === expertise ? "default" : "outline"
                }
                className="cursor-pointer hover:bg-primary/10"
    ...
    >
      ...
      <div className="px-6 pt-4">
        <div className="flex items-center gap-6 text-sm text-muted-foreground">
          ...
          <div className="flex items-center gap-2">
            <span>
              主持人: {agents.filter((a) => a.role === "moderator").length} 个
            </span>
          </div>
          <div className="flex items-center gap-2">
            <span>
              参与者: {agents.filter((a) => a.role === "participant").length} 个
            </span>
          </div>
        </div>
      </div>
