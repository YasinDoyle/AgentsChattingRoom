...
export function AgentDetailPage() {
  ...
  const editTab =
    (searchParams.get("tab") as "configure" | "ai-create" | null) ||
    "configure";
  const [sidebarTab, setSidebarTab] = useState<"configure" | "ai-create">(
    editTab,
  );

  // 切换到编辑模式
  const handleEnterEditMode = useCallback(
    (tab?: "configure" | "ai-create") => {
  ...
  const handleAgentUpdate = useCallback(
    (updatedAgentData: Omit<AgentDef, "id">) => {
      if (!agent) return;
  ...
  useEffectFromObservable(
    () =>
      combineLatest([agentId$, agents$]).pipe(
        map(([aId, aList]) => {
  ...
  if (mode === "profile") {
    return (
      <div className="h-full w-full flex overflow-hidden">
        ...
        {/* 右侧智能体预览聊天区 */}
        <AgentPreviewChat className="w-1/2" agentDef={agent} />
      </div>
    );
  }
      ...
      <div className="w-1/2 border-r flex flex-col">
        ...
        <div className="p-6 border-b">
            ...
            <div className="flex items-center gap-4 flex-1 min-w-0">
              <div className="relative">
                <SmartAvatar
                  ...
                  fallback={
                    <span className="bg-gradient-to-br from-primary/20 to-primary/40">
                      {agent.name?.[0] || "?"}
                    </span>
                  }
                />
                ...
                >
                  <roleConfig.icon
                    className={cn("w-2.5 h-2.5", roleConfig.color)}
                  />
                </div>
              </div>
          ...
          <Tabs
            value={sidebarTab}
            onValueChange={(value) =>
              setSidebarTab(value as "configure" | "ai-create")
            }
          >
