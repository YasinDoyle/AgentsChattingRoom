  ...
  async list(): Promise<T[]> {
    return this.executeTransaction("readonly", (store) => {
      return new Promise((resolve, reject) => {
        ...
        request.onerror = () => {
          reject(
            new Error(`Failed to get all data: ${request.error?.message}`),
          );
        };
      });
    });
        ...
        dataArray.forEach((data, index) => {
          ...
          request.onerror = () => {
            if (!hasError) {
              ...
              reject(
                new Error(
                  `Failed to create item at index ${index}: ${request.error?.message}`,
                ),
              );
            }
          };
    ...
    return this.executeTransaction("readwrite", (store) => {
      return new Promise((resolve, reject) => {
        ...
        getRequest.onsuccess = () => {
          ...
          putRequest.onerror = () => {
            reject(
              new Error(`Failed to update item: ${putRequest.error?.message}`),
            );
          };
        };
        ...
        getRequest.onerror = () => {
          reject(
            new Error(
              `Failed to get item for update: ${getRequest.error?.message}`,
            ),
          );
        };
      });
  ...
  static async listDatabases(): Promise<string[]> {
    ...
    if ("databases" in indexedDB) {
      try {
        ...
        console.warn(
          "[IndexedDBProvider] databases() API not supported, falling back to localStorage:",
          error,
        );
      }
    } else {
    ...
    try {
      ...
      console.warn(
        "[IndexedDBProvider] Failed to get database list from localStorage:",
        error,
      );
      return [];
    }
  ...
  private static removeDatabaseFromList(dbName: string): void {
    try {
      ...
      const filteredDatabases = databases.filter(
        (name: string) => name !== dbName,
      );
      localStorage.setItem(
        "indexedDB_database_list",
        JSON.stringify(filteredDatabases),
      );
    } catch (error) {
