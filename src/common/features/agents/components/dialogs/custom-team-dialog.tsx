...
export const CustomTeamDialogContent: React.FC<
  CustomTeamDialogContentProps
> = ({ agents, initialSelected, onConfirm, onCancel }) => {
  const [selectedMembers, setSelectedMembers] =
    useState<CustomTeamMember[]>(initialSelected);
  ...
  const handleAgentSelect = (agent: AgentDef, selected: boolean) => {
    if (selected) {
      setSelectedMembers((prev) => [
        ...prev,
        { agentId: agent.id, isAutoReply: true },
      ]);
    } else {
  ...
  const toggleAutoReply = (agentId: string) => {
    setSelectedMembers((prev) =>
      prev.map((m) =>
        m.agentId === agentId ? { ...m, isAutoReply: !m.isAutoReply } : m,
      ),
    );
  };
