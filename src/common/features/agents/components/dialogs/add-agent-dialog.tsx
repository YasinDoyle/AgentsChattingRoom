  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-none border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80">
        <div
          className={cn("flex items-center gap-4", isMobile ? "p-3" : "p-4")}
        >
          ...
          <Button
            ...
            disabled={isLoading}
            className={cn("px-4 shrink-0", isMobile ? "h-8 text-xs" : "h-9")}
          >
            {isLoading ? (
              <Loader2
                className={cn(
              ...
              <PlusCircle
                className={cn("mr-2", isMobile ? "w-3 h-3" : "w-4 h-4")}
              />
            )}
            添加 Agent
          </Button>
...
export function useAddAgentDialog() {
  ...
  const openAddAgentDialog = useCallback(() => {
    modal.show({
      ...
      className: cn(
        "overflow-hidden",
        isMobile ? "w-[95vw] h-[90vh]" : "sm:max-w-3xl sm:h-[85vh]",
      ),
      // 不需要底部按钮
      showFooter: false,
    });
