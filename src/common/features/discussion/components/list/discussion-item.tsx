...
export function DiscussionItem({ discussion, isActive }: DiscussionItemProps) {
  ...
  useEffect(() => {
    presenter.discussionMembers
      .getMembersForDiscussion(discussion.id)
      .then(setMembers);
  ...
  const handleConfirmEdit = async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (editValue.trim()) {
      await presenter.discussions.update(discussion.id, {
      ...
      onClick={(e) => {
        if (
          (e.target as HTMLElement).closest(
            ".discussion-actions, .editing-actions",
          )
        ) {
          return;
        }
