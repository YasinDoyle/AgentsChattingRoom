...
import {
  useMessageList,
  type MessageListRef,
} from "@/core/hooks/use-message-list";
...
export const MessageListDesktop = forwardRef<
  MessageListRef,
  MessageListDesktopProps
>(function MessageListDesktop({ className, scrollButtonThreshold = 200 }, ref) {
  ...
  const handleEditAgentWithAI = useCallback(
    (agent: AgentDef) => {
  ...
  return (
    <div className="relative h-full">
      ...
      >
        ...
        >
          <div className={cn("py-4")} ref={messagesContainerRef}>
            <div className="space-y-1 px-4">
              {reorganizedMessages.map((message, index) => {
                // 获取前一条消息的时间戳
                const previousMessage =
                  index > 0 ? reorganizedMessages[index - 1] : null;
                ...
                const isResponding =
                  message.agentId === respondingAgentId && isLastMessage;
                ...
                return (
                  <MessageItemWechat
                    ...
                    message={message}
                    agentInfo={{
                      getName: presenter.agents.getAgentName,
                      getAvatar: presenter.agents.getAgentAvatar,
                    }}
                    agent={agent}
