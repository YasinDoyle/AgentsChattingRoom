...
export function useAgentChatPageHelper() {
   ...
   * @param options 配置选项
   */
  const enterAgentChat = useCallback(
    async (
      agent: AgentDef,
      options?: {
        /** 是否强制新建空间 */
      ...
      const { forceNew = false, customTitle, autoReply = true } = options || {};

      // 1. 如果不强制新建，先查找现有空间
      if (!forceNew) {
        for (const d of discussions || []) {
          const members =
            await presenter.discussionMembers.getMembersForDiscussion(d.id);
          // 匹配条件：只有1个成员且是该AI
