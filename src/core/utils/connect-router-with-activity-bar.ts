...
export function createActivityBarToRouterMap(items: RouteConfig[]) {
  ...
  function processRoute(route: RouteConfig) {
    ...
    const primaryPath =
      route.routerPath || (route.routerPaths && route.routerPaths[0]);
    if (primaryPath) {
...
function findMatchingRoute(
  ...
  const sortedRoutes = [...routes].sort((a, b) => {
    const aHasParams =
      (a.routerPath?.includes(":") ||
        a.routerPaths?.some((p) => p.includes(":"))) ??
      false;
    const bHasParams =
      (b.routerPath?.includes(":") ||
        b.routerPaths?.some((p) => p.includes(":"))) ??
      false;
    const aHasWildcard =
      (a.routerPath?.includes("*") ||
        a.routerPaths?.some((p) => p.includes("*"))) ??
