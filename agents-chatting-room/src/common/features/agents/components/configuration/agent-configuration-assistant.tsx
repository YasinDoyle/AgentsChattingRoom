import {
  AgentChatInput,
  AgentChatMessages,
} from "@/common/features/chat/components/agent-chat";
...
function AgentConfigurationAssistantInner({
  onAgentCreate,
  className,
  editingAgent,
}: AgentConfigurationAssistantProps) {
  // 使用拆分的工具Hook
  useAgentConfigurationTools(onAgentCreate, editingAgent);

  // 使用useMemo缓存agentCreatorDef，避免每次渲染重新创建
  const agentCreatorDef = useMemo(
    (): Omit<AgentDef, "id"> => ({
      name: "Agent Creator",
...
export function AgentConfigurationAssistant(
  props: AgentConfigurationAssistantProps,
) {
  return (
