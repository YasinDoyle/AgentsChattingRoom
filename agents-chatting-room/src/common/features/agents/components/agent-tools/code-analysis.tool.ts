  ...
  parameters: {
    ...
    properties: {
      type: {
        ...
        description:
          "分析类型：structure（结构分析）、complexity（复杂度分析）、quality（质量分析）、summary（代码摘要）",
      },
      code: {
        type: "string",
      ...
      switch (type) {
        case "structure": {
          ...
          const functions = lines.filter((line: string) =>
            /function\s+\w+|const\s+\w+\s*=\s*\(|let\s+\w+\s*=\s*\(|var\s+\w+\s*=\s*\(|class\s+\w+/.test(
              line,
            ),
          ).length;
          ...
          const comments = lines.filter((line: string) =>
            /\/\/|\/\*|\*/.test(line),
          ).length;
        ...
        case "complexity": {
          // 简单的复杂度分析
          const lines = code.split("\n");
          const cyclomaticComplexity = lines.reduce(
            (complexity: number, line: string) => {
              if (
                /if\s*\(|else\s*if|for\s*\(|while\s*\(|switch\s*\(|case\s+|catch\s*\(|&&|\|\|/.test(
                  line,
                )
              ) {
                return complexity + 1;
                ...
                complexity:
                  cyclomaticComplexity <= 5
                    ...
                    : cyclomaticComplexity <= 10
                      ...
                      : "高",
                recommendations:
                  cyclomaticComplexity > 10 ? "建议拆分复杂函数" : "复杂度适中",
              },
          ...
          const functions = lines
            ...
            .map((line: string) => {
              const match = line.match(
                /(?:function\s+(\w+)|(?:const|let|var)\s+(\w+)|class\s+(\w+))/,
              );
