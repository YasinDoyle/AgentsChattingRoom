...
export function createUpdateAgentTool(
  ...
  function UpdateAgentToolRender({
    toolInvocation,
    onResult,
  }: {
    toolInvocation: ToolInvocationLike;
    onResult: (result: UpdateAgentToolResult) => void;
  }) {
  ...
  return {
    name: "updateAgent",
    description:
      "更新或创建智能体配置。当用户要求创建或修改智能体时使用此工具。",
    parameters: {
      type: "object",
      properties: {
        name: {
        ...
        role: {
          ...
          description:
            "智能体的角色类型：participant（参与者）或moderator（主持人）",
        },
        expertise: {
          type: "array",
          items: {
    ...
    async execute(toolCall: ToolCall) {
      try {
        ...
        const agentConfig: Omit<AgentDef, "id"> = {
          ...
          avatar:
            args.avatar ||
            `https://api.dicebear.com/7.x/bottts/svg?seed=${encodeURIComponent(args.name)}`,
        };
        if (onAgentCreate) {
