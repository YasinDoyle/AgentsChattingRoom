...
export class IframeManager {
  ...
  getIframeByKey(key: string): IframeInfo | undefined {
    return Array.from(this.iframes.values()).find(
      (iframe) => iframe.key === key,
    );
  }
  ...
  cleanupExpired(maxAge: number = 30 * 60 * 1000): void {
    // 默认30分钟
    const now = Date.now();
  ...
  injectScript(id: string, script: string): boolean {
    ...
    if (iframeInfo?.element?.contentDocument) {
      const scriptElement =
        iframeInfo.element.contentDocument.createElement("script");
...
export function useIframeManager() {
  const [manager] = useState<IframeManager>(() => new IframeManager());
  const [iframes, setIframes] = useState<IframeInfo[]>([]);

  // 创建 iframe
  const createIframe = useCallback(
    (key: string, type: IframeInfo["type"] = "custom") => {
  ...
  const registerElement = useCallback(
    (id: string, element: HTMLIFrameElement) => {
