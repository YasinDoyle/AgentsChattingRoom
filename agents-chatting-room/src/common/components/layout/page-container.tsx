    ...
    <div className={cn("h-full flex flex-col flex-grow-1", className)}>
      ...
      <div className="flex-none border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80">
        <div className="p-4 lg:p-6">
          ...
          {/* 筛选器内容 */}
          {filters && <div className="mt-4 pt-4 border-t">{filters}</div>}
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 flex w-full">
        {/* 侧边栏 */}
        {sidebar && (
          <div className="w-80 flex-none border-r bg-muted/20">{sidebar}</div>
        )}

        {/* 主内容区域 */}
        <div className="flex-1 min-w-0 w-full overflow-auto">{children}</div>
      </div>
    </div>
  );
};
